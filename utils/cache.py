...
def _increment_usage(video_id: str) -> int:
    ...
    if current is None:
        cache.set(key, 1)
        logger.debug(f"_increment_usage: video_id={video_id}, usage=1")
        return 1
    try:
        cache.incr(key)
        logger.debug(f"_increment_usage: video_id={video_id}, usage={current + 1}")
        return current + 1
    except Exception:
        new = current + 1
        cache.set(key, new)
        logger.warning(f"_increment_usage fallback: video_id={video_id}, usage={new}")
        return new
...
def _calculate_ttl(video_id: str) -> int:
    ...
    if usage >= HIGH_USAGE_THRESHOLD * 3:
        ...
        ttl = DEFAULT_TTL
    logger.debug(f"_calculate_ttl: video_id={video_id}, usage={usage}, ttl={ttl}")
    return ttl
...
def _evict_least_used(max_bytes: int = MAX_CACHE_SIZE_BYTES):
    ...
    logger.info(f"[cache] КЭШ ПЕРЕПОЛНЕН: {current_size // 1024**2} МБ > {max_bytes // 1024**2} МБ")
    logger.info("[cache] Очистка...")
    ...
    for usage, size, video_id, filepath in candidates:
        ...
        try:
            ...
            current_size -= size
            logger.info(f"[cache] УДАЛЕНО: {Path(filepath).name} ({size // 1024**2} МБ, usage: {usage})")
        except Exception as e:
            logger.error(f"[cache] Ошибка удаления {filepath}: {e}")

    logger.info(f"[cache] Размер после очистки: {current_size // 1024**2} МБ")
...
def get_cached_filepath(video_id: str) -> Optional[str]:
    ...
    if filepath and Path(filepath).exists():
        ...
        logger.debug(f"[cache] HIT: {filepath} (usage: {_get_usage_count(video_id)})")
        return filepath
    if filepath:
        cache.delete(f"path:{video_id}")
        cache.delete(f"usage:{video_id}")
...
def set_cached_filepath(video_id: str, filepath: str):
    usage = cache.get(f"usage:{video_id}") or 1
    cache.set(f"usage:{video_id}", usage)
    ttl = _calculate_ttl(video_id)
    cache.set(f"path:{video_id}", filepath, expire=ttl)
    logger.debug(f"[cache] SET: {filepath}, usage={usage}, ttl={ttl}")
    _evict_least_used()

def clear_expired():
    removed = cache.expire()
    if removed:
        logger.info(f"[cache] Удалено по TTL: {removed} записей")
